...
if i == 2
    mu_simLarge = mu_sim > -0.4;
    rates_simSmall = rates_sim < 0.05;
    outlier = mu_simLarge + rates_simSmall;
    mu_sim = mu_sim(outlier < 2);
    rates_sim = rates_sim(outlier < 2);
end
...
if i == 2
    dp_testNew = [dp_test(1:k-1) 0 dp_test(k:end)];
    everyNeuronGainVect = [everyNeuronGainVect dp_testNew];
else
...
weightMat = h5read('weights_09_02_2020_freeze322ad530-ed3c-11ea-0eb5-6f686438d8a4.h5','/weights');
...
weightMatInput = weightMat(1:5000,5001:end);
 ...
 for i = 1:5000%length(idxPosIntoWeights) %Pad
     for j=1:5000%length(idxPosIntoWeights) %Pad
         if i < 4001
            synScale = 0.3; %was 0.4!
         else
            synScale = 0.15; %was 0.1!
         end
